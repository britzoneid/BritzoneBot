import { CommandInteraction, VoiceBasedChannel, VoiceChannel, ChannelType } from 'discord.js';
        ...
        try {
          ...
          if (guildRoom.members && guildRoom.members.size > 0) {
            for (const [memberId, member] of guildRoom.members) {
              ...
              try {
                await distributionService.moveUserToRoom(member, mainChannel as VoiceChannel);
          ...
          if (!steps[roomDeletedKey]) {
            try {
              await roomService.deleteRoom(guildRoom, 'Breakout room ended and members moved back to main room');
...
export default new EndOperation();
